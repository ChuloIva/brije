...
def ai_turn(ai_name, conversation, model, system_prompt, gui=None, is_branch=False, branch_output=None):
    ...
    try:
        response = None
        if "gemma-3-4b-probes" in model_id.lower():
            print(f"\n--- Prompt to {model} (Gemma with Probes) ---")
            response = call_gemma_api(prompt, full_context, model_id, system_prompt)
    ...
    if response:
        ...
        if isinstance(response, dict):
            if "predictions" in response and "content" in response:
                # Handle Gemma with probes response
                conversation.append({
                    "role": "assistant",
                    "model": model,
                    ...
                    "raw_content": json.dumps({
                        "role": "assistant",
                        "content": response["content"],
                        "cognitive_actions": response["predictions"]
                    }, indent=2)
                })
                # Display response and update probe visualization
                if gui:
                    gui.append_text(f"\n{ai_name} ({model}):\n\n{response['content']}\n")
                    # Update probe panel if it exists
                    if hasattr(gui, 'update_probe_display'):
                        gui.update_probe_display(ai_name, response['predictions'])
                elif branch_output:
                    branch_output(f"\n{ai_name} ({model}):\n\n{response['content']}\n")
            elif "display" in response and "content" in response:
