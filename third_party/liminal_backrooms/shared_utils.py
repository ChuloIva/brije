...
# Import Gemma with probes (lazy loading)
_gemma_with_probes = None

def get_gemma_with_probes():
    """Lazy load Gemma with probes"""
    global _gemma_with_probes
    if _gemma_with_probes is None:
        from gemma_probes import get_gemma_instance
        _gemma_with_probes = get_gemma_instance()
    return _gemma_with_probes
...
def call_gemma_api(prompt, conversation_history, model, system_prompt):
    """Call Gemma 3 4B with cognitive action probes"""
    try:
        # Get Gemma instance
        gemma = get_gemma_with_probes()
        ...
        # Generate response
        response_text = gemma.generate(full_prompt, system_prompt, max_new_tokens=512)
